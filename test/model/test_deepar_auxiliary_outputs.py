...
def test_shape():
    ...
    train_model = estimator.train_model(train_ds)

    # todo adapt loader to anomaly detection use-case
    batch_size = 2
    training_data_loader = TrainDataLoader(
        dataset=train_ds,
        transform=train_model.transformation,
    ...
    for data_entry in islice(training_data_loader, 1):
        input_names = get_hybrid_forward_input_names(train_model.trained_net)

        loss, likelihoods, *distr_args = train_model.trained_net(

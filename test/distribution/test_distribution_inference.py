...
def test_multivariate_gaussian() -> None:
    ...
    mu_hat, L_hat = maximum_likelihood_estimate_sgd(
        ...
        samples,
        init_biases=None,  # todo we would need to rework biases a bit to use it in the datasets case
        hybridize=False,
...
@pytest.mark.timeout(10)
def test_multivariate_independent_gaussian() -> None:
    num_samples = 2000
    dim = 2

    mu = np.arange(0, dim) / float(dim)

    L_diag = np.arange(1, dim + 1)
    L_low = 0.1 * np.ones((dim, dim)) * np.tri(dim, k=-1)
    L = np.diag(L_diag) + L_low
    Sigma = L.dot(L.transpose())

    distr = MultivariateIndependentGaussian(
        mu=mx.nd.array(mu), sigma=mx.nd.array(L_diag), dim=dim
    )
    ...
    distr = MultivariateIndependentGaussian(
        mu=mx.nd.array([mu_hat]), sigma=mx.nd.array([sigma_hat]), dim=dim
    )

...
class LowrankMultivariateGaussian(Distribution):
    ...
    dim
        Dimension of the distribution's support, if not specified then self.variance is not available
    """
    ...
    def sample_rep(self, num_samples: int = None) -> Tensor:
        r"""
        Draw samples from the datasets Gaussian distribution:
...
class LowrankMultivariateGaussianOutput(DistributionOutput):
    ...
    def distribution(self, distr_args, scale=None, **kwargs) -> Distribution:
        distr = LowrankMultivariateGaussian(self.rank, *distr_args, self.dim)
        if scale is None:
            return distr
        else:

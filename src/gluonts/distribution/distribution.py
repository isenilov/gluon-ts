...
class Distribution:
    ...
    def _sample_variance(
        ...
        Parameters
        ----------
        mean (batch_size, seq_len, num_samples, target_dim)
        samples (batch_size, seq_len, num_samples, target_dim)
        num_samples
        is_two_dimensional
        ...
        """
        F = getF(samples)
        residual = samples - mean.expand_dims(axis=0)

        # todo this only works with fixed dimensions
        # (batch_size, seq_len, num_samples, target_dim)
        if not is_two_dimensional:
            residual = residual.transpose(axes=(1, 2, 0, 3))
        # (batch_size, seq_len, target_dim, target_dim)
        return (
            F.linalg_gemm2(residual, residual, transpose_a=True) / num_samples
        )
    ...
    def sample_variance(
        ...
        """
        Estimates covariance with sampling, useful to get covariance in transformed
        distribution when no form is known.
        """
        # (num_samples, batch_size, seq_len, target_dim)
        # Fast sampling does not work for empirical CDF for now:
        if fast_sampling:
            samples = self.sample(num_samples=num_samples)
        else:
            samples = [
                self.sample().expand_dims(axis=0) for _ in range(num_samples)
            ]
            F = getF(samples[0])
            samples = F.concat(*samples, dim=0)

        return Distribution._sample_variance(
            mean=self.mean,
            samples=samples,

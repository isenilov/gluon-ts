...
from gluonts.distribution.searchsorted import *
from gluonts.distribution.erf import *
...
class _Sigmoid(Bijection):
    def f(self, x):
        return x.sigmoid()

    def f_inv(self, y):
        # y_clipped = y.clip(1.0e-20, 0.99999994)  # largest float32 below 1
        y_clipped = y.clip(1.0e-6, 1.0 - 1.0e-6)
        return y_clipped.log() - (-y_clipped).log1p()

    def log_abs_det_jac(self, x, y):
        y_clipped = y.clip(1.0e-20, 0.99999994)  # largest float32 below 1
        y_clipped = y.clip(1.0e-6, 1.0 - 1.0e-6)
        # alternative: return -(1.0/y_clipped + 1.0/(1-y_clipped)).log()
        return y_clipped.log() + (-y_clipped).log1p()
...
class GaussianCDF(Bijection):
    ...
    def f(self, x):
        F = getF(x)
        return 0.5 * (1.0 + F.Custom(x / GaussianCDF.SQRT2, op_type='erf'))

    def f_inv(self, y):
        F = getF(y)
        y_clipped = y.clip(1.0e-6, 1.0 - 1.0e-6)
        return GaussianCDF.SQRT2 * F.Custom(
            2.0 * y_clipped - 1.0, op_type='erfinv'
        )

    def log_abs_det_jac(self, x, y):
        return -0.5 * (GaussianCDF.LOG2PI + x.square())

...
class MultivariateGaussian(Distribution):
    ...
    def __init__(
        self, mu: Tensor, L: Tensor, float_type: DType = np.float32
    ) -> None:
        self.mu = mu
        self.L = L
    ...
    def variance(self) -> Tensor:
        return getF(self.L).linalg_gemm2(self.L, self.L, transpose_b=True)

    def sample_rep(self, num_samples: Optional[int] = None) -> Tensor:
        r"""
        Draw samples from the datasets Gaussian distributions.

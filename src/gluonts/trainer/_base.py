...
class Trainer:
    ...
    def __call__(
        ...
        float
    ]:  # TODO: we may want to return some training information here
        self.halt = False
                ...
                for epoch_no in range(self.epochs):
                    ...
                    with tqdm(train_iter) as it:
                        for batch_no, data_entry in enumerate(it, start=1):
                            ...
                            if first_batch:
                                # the network needs to be initialized (for instance after a forward otherwise,
                                # some parameters are not counted
                                net_name = type(net).__name__
                                num_model_param = self.count_model_params(net)

                                # if not self.hybridize:
                                #     net.summary(*inputs)
                            ...
                            # avg_epoch_loss = loss_value(epoch_loss) / dim
                            avg_epoch_loss = loss_value(epoch_loss)

                            if hasattr(net, 'target_dim'):
                                avg_epoch_loss /= net.target_dim
                    ...
                    # check and log epoch loss
                    if not np.isfinite(loss_value(epoch_loss)):
                        logging.warning(
                            "Encountered invalid loss value! Try reducing the learning rate "
                            "or try a different likelihood. Loading the best model found."
                        )
                        break
                    else:
